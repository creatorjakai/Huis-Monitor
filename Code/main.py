...
pygame.display.set_caption("Huis Monitor v26.0")
...
def searchfordeviceonip():
    global searchip, randomip, menu, errortext, fps, start2, start3, start4
    ...
    if randomsearch:
                    ...
                    for rndport in range(startport, 8001):
                        ...
                        for event in pygame.event.get():
                            if event.type == pygame.QUIT:
                                ...
                                sys.exit()
                        try:
                            socket.create_connection((randomip, rndport), timeout=0.1)
                            return chooseip()
                        except OSError:
                            pass
                        time.sleep(0.01)
        ...
        try:
            socket.create_connection((searchip, searchport), timeout=10)
            menu = "temperatuur"
            return main()
        except OSError:
